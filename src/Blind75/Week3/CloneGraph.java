...
class Solution {
    public Node cloneGraph(Node node) {
        ...
        Node root = visited.get(node.val);
        root.neighbors = new LinkedList<>();

        while (!queue.isEmpty()) {
            Node tempNode = queue.pollFirst();
            Node currNode = visited.get(tempNode.val);
            ...
            for (Node neighbour : tempNode.neighbors) {
                if (!visited.containsKey(neighbour.val)) {
                    ...
                    visited.put(neighbour.val, newNode);
                }
                currNode.neighbors.add(visited.get(neighbour.val));
            }
        }
